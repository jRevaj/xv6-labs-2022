...
struct bucket
{
  struct spinlock lock; // buckets lock
  struct buf head;  // buckets head
};
...
struct {
  ...
  struct buf buf[NBUF];
  uint ticks[NBUF]; // last usage time

  // Linked list of all buffers, through prev/next.
  // Sorted by how recently the buffer was used.
  // head.next is most recent, head.prev is least.
  // struct buf head;
  struct bucket bucket[NBUCKET]; // replace linked list of all buffers with list of all buckets
} bcache;
